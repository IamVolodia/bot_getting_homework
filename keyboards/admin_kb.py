...
from models.functions import get_user, get_all_users_by_group, get_all_subjects_by_group, get_subject_with_homework_for_date, get_group_by_group_id
...
def create_FSM_all_subjects_keyboard(user_id, date) -> InlineKeyboardMarkup:
    # Получаем кнопки меню
    buttons = LEXICON()['admin']['admin_menu_calendar']['admin_menu_date']['FSM_add_homework']['buttons']
    # Получаем год, месяц, день
...
# Инлай клавиатура для меню admin, где перечислены предметы группы
def create_admin_menu_group_keyboard(user_id) -> InlineKeyboardMarkup:
    # Получаем кнопки меню
    buttons = LEXICON()['admin']['admin_menu_group']['buttons']
    # Получаем group_id
    group_id = get_user(user_id)[-1][-1]
    # Получаем данные группы
    group_name, group_password = get_group_by_group_id(group_id)[-1][-2:]
    # Создаем клавиатуру
    kb_builder = InlineKeyboardBuilder()
    # Создаем кнопки названия группы и пароля
    kb_builder.row(InlineKeyboardButton(text=f"Название: {group_name}", callback_data='something'))
    kb_builder.row(InlineKeyboardButton(text=f"Пароль: {group_password}", callback_data='something'))
    # Разделитьель информации от кнопок
    kb_builder.row(InlineKeyboardButton(text=LEXICON()['general']['fence'], callback_data="fence"))
    # Генерируем кнопки
    kb_builder.row(*[InlineKeyboardButton(text=text, callback_data=button) for button, text in buttons.items()], width=1)
