...
# Фильтр который ловит callback на предмет по которому есть дз в конкретной дате
...
# Фильтр который проверят сообщение на соответсвие его типа тому, что возможно сохранить в бд
class IsCorrectMessageToAdd(BaseFilter):
    async def __call__(self, message: Message) -> bool:
        if message.text or message.photo or message.audio or message.video or message.document or message.voice:
            return True
...
# Фильтр который ловит callback на дату
class IsRightDateForUser(BaseFilter):
    async def __call__(self, callback: CallbackQuery) -> bool:
        return callback.data.startswith('user_date_') and all(i.isdigit() for i in callback.data.split('_')[-3:])
    

# Фильтр который ловит callback на предмет по которому есть дз в конкретной дате
class IsUserSubjectFromCalendar(BaseFilter):
    async def __call__(self, callback: CallbackQuery) -> bool:
        return callback.data.startswith('user_date_subject_id_')
